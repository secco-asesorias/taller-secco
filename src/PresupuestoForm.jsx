...
import { calcularTotalesCotizacion, guardarCotizacion, actualizarEstadoCotizacion, aprobarCotizacion, rechazarCotizacion, precioFinalConMargen } from './lib/supabase'
import { generarPDFPresupuestoCliente, generarPDFPresupuestoInterno } from './lib/pdfPresupuesto'
...
export default function PresupuestoForm({ cotizacionInicial, onVolver, onAbrirOT }) {
  ...
  const scheduleAutosave = useCallback((r, mo, nd, ndt, nn, nni, nt, mg, ht, cht, vhcm) => {
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        await guardarCotizacion(cotizacion.id, {
  ...
  async function handleAction(newStatus) {
    ...
    try {
      ...
      if (newStatus !== status) {
        await actualizarEstadoCotizacion(cotizacion.id, newStatus)
        setStatus(newStatus)
      }
      setSaveStatus('saved')
  ...
  async function handleAprobar() {
    ...
    try {
      ...
      const ot = await aprobarCotizacion(cotizacion.id)
      setStatus('aprobada')
  ...
  async function handleRechazar() {
    ...
    setLoading(true)
    try {
      await rechazarCotizacion(cotizacion.id, motivoRechazo)
      setStatus('rechazada')
