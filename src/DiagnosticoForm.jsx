...
export default function DiagnosticoForm({ onVolver }) {
  ...
  async function guardarSeccionActual(num) {
    if (!supabaseConfigurado() || !diagnosticoData.diagnostico_id) return
    await guardarChecklist(diagnosticoData.diagnostico_id, itemsDeSeccion(num))
    if (diagnosticoData.status === 'pendiente') {
      await actualizarDiagnostico(diagnosticoData.diagnostico_id, { status: 'proceso' })
  ...
  async function finish() {
    ...
    try {
      ...
      if (supabaseConfigurado() && diagnosticoData.diagnostico_id) {
        await guardarRepuestos(diagnosticoData.diagnostico_id, diagnosticoData.repuestos)
        await actualizarDiagnostico(diagnosticoData.diagnostico_id, {
          status: 'listo',
