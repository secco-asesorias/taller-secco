...
import { notificarInicioMantencion, notificarMantencionFinalizada } from './lib/emailNotifications'
...
export default function OTForm({ otInicial, onVolver }) {
  ...
  async function handleAsignar() {
    ...
    setLoading(true)
    try {
      await asignarTecnico(ot.id, tecnico)
      setStatus('asignada')
      const updated = await cargarOTCompleta(ot.id)
      setOt(updated)
  ...
  async function handleAvanzar() {
    ...
    setLoading(true)
    try {
      await avanzarEstadoOT(ot.id, next)
      setStatus(next)
      const updated = await cargarOTCompleta(ot.id)
      setOt(updated)
      if (next === 'en_proceso') await notificarInicioMantencion(updated)
      if (next === 'finalizada') await notificarMantencionFinalizada(updated)
    } catch (e) { alert(`Error: ${e.message}`) }
    finally { setLoading(false) }
  }
          ...
          <>
            ...
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #F2F2F2' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button type="button" onClick={handleAsignar} disabled={loading || !tecnico}
                  style={{ padding: '8px 18px', fontSize: 13, fontWeight: 600, background: '#a98225', color: '#FFF', border: 'none', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit', opacity: !tecnico ? 0.5 : 1 }}>
                  {loading ? '…' : 'Asignar'}
                </button>
              </div>
            </div>
