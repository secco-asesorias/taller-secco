...
export function validarSeccion3(datos) {
  ...
  if (!datos.foto_km) errores.foto_km = 'La foto del kilometraje es obligatoria'
  if (!datos.combustible) errores.combustible = 'Selecciona el nivel de combustible'
  if (!datos.foto_combustible) errores.foto_combustible = 'La foto del combustible es obligatoria'
  if (!datos.llaves || datos.llaves < 0) errores.llaves = 'Indica la cantidad de llaves'
  return errores
}
